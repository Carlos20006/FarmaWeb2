...
$id = $_GET['id'];
$medicamento = $controller->getById($id);
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if (!isset($_POST['csrf_token']) || !validate_csrf($_POST['csrf_token'])) {
        ...
        $error = 'El stock m&iacute;nimo no puede ser negativo';
    } elseif(!preg_match("/^[A-Za-záéíóúñÑ ]+$/", $_POST['nombre'])){
        $error = 'El nombre solo debe contener letras y espacios';
    } elseif(empty($_POST['presentacion'])){
        $error = 'Debe seleccionar una presentaci&oacute;n';
    } else {
        $data = [
            'nombre' => $_POST['nombre'],
...
<div class="w-full max-w-2xl bg-surface-container-lowest rounded-[32px] p-8 md:p-12 shadow-[0_4px_30px_rgba(0,0,0,0.04)]">
    <div class="flex flex-col items-center mb-8">
        ...
        <h1 class="font-headline-lg text-headline-lg text-on-surface text-center">Editar Medicamento</h1>
        <p class="font-body-md text-body-md text-on-surface-variant mt-2 text-center">Modifique los datos del medicamento.</p>
    </div>
    ...
    <form method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="flex flex-col">
                ...
                <div class="relative">
                    <select class="w-full h-14 pl-4 pr-10 bg-surface-container-low rounded-xl border-none focus:ring-2 focus:ring-primary-container focus:bg-white appearance-none transition-all duration-300 font-body-md text-body-md outline-none" id="presentacion" name="presentacion" required>
                        <option value="<?php echo htmlspecialchars($medicamento['presentacion']); ?>"><?php echo htmlspecialchars($medicamento['presentacion']); ?></option>
            ...
            <div class="floating-label-group">
                <input class="w-full h-14 px-4 pt-2 bg-surface-container-low rounded-xl border-none focus:ring-2 focus:ring-primary-container focus:bg-white transition-all duration-300 font-body-md text-body-md outline-none" id="precio" name="precio" placeholder=" " type="number" step="0.01" min="0" value="<?php echo htmlspecialchars($medicamento['precio']); ?>" required/>
                <label class="font-body-md text-body-md text-outline" for="precio">Precio Unitario ($)</label>
            </div>
            <div class="floating-label-group">
                <input class="w-full h-14 px-4 pt-2 bg-surface-container-low rounded-xl border-none focus:ring-2 focus:ring-primary-container focus:bg-white transition-all duration-300 font-body-md text-body-md outline-none" id="stock_minimo" name="stock_minimo" placeholder=" " type="number" min="0" value="<?php echo htmlspecialchars($medicamento['stock_minimo']); ?>"/>
